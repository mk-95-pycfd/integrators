...
def RK54_taylor_vortex (steps = 3,return_stability=False,name='regular',guess=None,project=[1,1,1,1],alpha=0.99,post_projection=False):
    # problem description
    ...
    while count < tend:
        ...
        elif count <= 5: # compute pressures for 2 time steps
            d2 = 1
            d3 = 1
            d4 = 1
            d5 = 1
        ...
        if d2 == 1:
            ...
            if guess == "third":
                p_approx = ((25 *a21)/12) *pn + (-((23 * a21)/12))*pnm1 +((13 *a21)/12)*pnm2 + (-((a21)/4))*pnm3
        ...
        if d3 == 1:
            ...
            if guess == "third":
                c3 = a32 + a31
                p_approx =  ((5/12)*a21*(5 + 7*a32)) * pn + (-(23/12) *(a21 + 3 * a21 * a32))*pnm1 +((1/12)*a21*(13 + 45*a32))*pnm2 + (-(1/12)*a21*(3 + 11*a32))*pnm3
        ...
        uhnp1 = u + dt*b1*(urhs1)  + dt*b2*(urhs2) + dt*b3*(urhs3) +  dt*b4*(urhs4) +  dt*b5*(f.urhs(u5,v5))
        vhnp1 = v + dt*b1*(vrhs1)  + dt*b2*(vrhs2) + dt*b3*(vrhs3) +  dt*b4*(vrhs4) +  dt*b5*(f.vrhs(u5,v5))
        ...
        new_press = (25/12) * press + (-23/12) * pn + (13/12) * pnm1 + (-1/4) * pnm2
